...
![](E:\Teaching\Introduction to data analysis\Lectures 2023-2024\Lý thuyết\Tiền xử lý dữ liệu trên R\h1.png)
![](E:\Teaching\Introduction to data analysis\Lectures 2023-2024\Lý thuyết\Tiền xử lý dữ liệu trên R\h2.png) Đoạn
lệnh sau cho ta một số thông tin khi truy cập vào véc tơ chỉ số lỗi của
$10$ trang đánh máy trên:
...
![](E:\Teaching\Introduction to data analysis\Lectures 2023-2024\Lý thuyết\Tiền xử lý dữ liệu trên R\h3.png)
...
![](E:\Teaching\Introduction to data analysis\Lectures 2023-2024\Lý thuyết\Tiền xử lý dữ liệu trên R\h4.png)
...
Lấy dữ liệu **mtcars** trong tạp chí Motor Trend US 1974. Trong dữ liệu này, biến “am” là kiểu hộp số được mã hóa với 2 giá trị 0 (automatic), 1(manual). Chúng ta sẽ mã hóa lại biến số am thành dạng nhân tố (factor) với tên lần lượt là “automatic” và “manual” và gán cho một biến số mới có tên là “trans”.
```{r}
data=mtcars
is.numeric(data$am)
...
Giả sử chúng ta muốn rời rạc hóa dữ liệu công suất của 32 dòng ô tô thành 3 nhóm: cao (H), trung bình (M), thấp (L) và gán các giá trị này vào một biến số mới có tên “power”. Ta sử dụng hàm **cut**:
```{r}
...
- Vì việc chọn mẫu là ngẫu nhiên nên có thể cùng một lệnh chọn mẫu nhưng với những lần chọn khác nhau cho ta kết quả là khác nhau. Trong trường hợp muốn cố định mẫu được chọn để dùng lại trong các lần sau, ta dùng hàm **set.seed(n)**, trong đó **n** là một số nguyên dương dùng để cố định mẫu.
...
- Hàm sample cho ta cách chọn mẫu từ một véc tơ. Trong trường hợp muốn chọn mẫu từ một nhóm đối tượng có dữ liệu chứa trong một bảng dữ liệu, đầu tiên ta áp dụng hàm **sample** cho véc tơ chỉ số thứ tự của các đối tượng, sau đó lấy từ bảng ra những quan sát có thứ tự đã chọn được.

Chẳng hạn với bảng **DuLieu** gồm thông tin của 5 sinh viên cho trong phần trước, để chọn ngẫu nhiên ra 3 sinh viên từ bảng này, ta thực hiện
như sau:
...
Sinh ngẫu nhiên hai vec tơ cùng độ dài $10$, tính tổng theo công thức $\text{tong} = x^2 + y$
```{r}
...
Viết hàm truyền vào một véc tơ x và tính tổng theo công thức $\text{tong} =\sum^{length(x)}_{i=1}x_k^k$ với $x_k$ là vị trí thứ $k$ của véc tơ $x$. (Ví dụ $x=c(2,3,4,5), \text{tong}=2^1+3^2+4^3+5^4$)
...
Trong R, ngoài việc sử dung vòng for để thực hiện thao tác lặp, ta còn có thể sử dụng các hàm **apply() , lapply() , sapply()**. 

Ưu điểm lớn nhất của các hàm này so với sử dụng vòng lặp **for** là giúp code trở nên
ngắn gọn hơn.
...
Điểm khác biệt của hàm trên so với hàm **lapply()** là hàm trả về vector hoặc matrix.
```{r}
